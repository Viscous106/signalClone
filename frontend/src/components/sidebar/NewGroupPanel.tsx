...
export function NewGroupPanel({ onBack }: { onBack: () => void }) {
  ...
  return (
    <div className="flex h-full flex-col">
      <PanelHeader
        title={step === "members" ? "New group" : "Name this group"}
        onBack={step === "members" ? onBack : () => setStep("members")}
      />
      ...
      {step === "members" ? (
        ...
        <>
          ...
          <footer className="flex items-center justify-end border-t border-edge px-4 py-3">
            <button
              onClick={create}
